...
class CentralControllerNode(Node):
    def __init__(self):
        ...
        # TEMP: run a 2-robot checkpoint with a single physical robot by mirroring its observation
        self.declare_parameter('single_robot_mode', False)
        ...
        self._single_robot_mode = bool(self.get_parameter('single_robot_mode').value)
        ...
        # TEMP single_robot_mode: load the 2-robot checkpoint by telling the model there are 2 robots;
        # observations will be mirrored from robot 0 at runtime.
        _model_num_robots = 2 if self._single_robot_mode else self.num_robots
        ...
        srm_tag = ' [SINGLE_ROBOT_MODE]' if self._single_robot_mode else ''
    ...
    def _build_pose_tensor(self) -> Optional[torch.Tensor]:
        ...
        x = torch.tensor(poses, dtype=torch.float32)
        # TEMP single_robot_mode: mirror robot 0 into a phantom robot 1 slot
        if self._single_robot_mode:
            x = x.repeat(2, 1)
        return x

    def _build_obs_tensors(self):
        n_model_robots = 2 if self._single_robot_mode else self.num_robots
        z_world = torch.zeros(n_model_robots, self.num_targets, 2)
        visible = torch.zeros(n_model_robots, self.num_targets, dtype=torch.bool)
        for i, obs in enumerate(self.obs_buf):
            ...
            for t in range(n):
                if obs.visible[t]:
                    ...
                    visible[i, t] = True
        # TEMP single_robot_mode: copy robot 0 observations into phantom robot 1 slot
        if self._single_robot_mode:
            z_world[1] = z_world[0]
            visible[1] = visible[0]
    ...
    def _publish_actions(self, actions: torch.Tensor):
        ...
        for i in range(self.num_robots):
            ...
            # TEMP single_robot_mode: model outputs 2 actions; only robot 0 is real — stop here
            if self._single_robot_mode:
                break
