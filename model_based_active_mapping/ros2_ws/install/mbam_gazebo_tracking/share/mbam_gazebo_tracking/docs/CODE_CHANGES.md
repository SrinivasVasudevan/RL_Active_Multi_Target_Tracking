...
- `ros2_ws/src/mbam_gazebo_tracking/mbam_gazebo_tracking/nodes/limo_central_coordinator_node.py`
  ...
  - Subscribes to per-robot reports, builds target tracks, estimates target velocity, runs MBAM policy inference, and publishes `cmd_vel`.
...
- `ros2_ws/src/mbam_gazebo_tracking/launch/run_mbam_limo_edge.launch.py`
  - Launches one LIMO-side observer node.
